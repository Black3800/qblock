...
from qblock.block_util import encode_secretkey, decode_secretkey, encode_pubkey, decode_pubkey, decode_signature
...
def handle_warm_insert(block_dict, hot, warm, meta, qblock):
    ...
    if not block.verify():
        qblock.log.insert_one({"status": "invalid block"})
        return 'invalid block'
    if block.hash()[:QBLOCK_DIFFICULTY] != b"0"*QBLOCK_DIFFICULTY:
    ...
    hot.insert_one(block_dict)
    warm.delete_one({"_id": block.hash()})
            ...
            with qblock.watch() as stream:
                ...
                for change in stream:
                    if change["operationType"] == "insert":
                        if change["ns"]["coll"] == "warm":
                            handle_warm_insert(change["fullDocument"], hot, warm, meta, qblock)
